...
import { ImprovementAnalyzer } from './improvement-analyzer';
...
import { evaluateOperationalSuccess } from './operational-success';
...
export const GOLDEN_TASKS: OperationalTask[] = [
...
const TRAINING_TASKS: OperationalTask[] = [
  ...GOLDEN_TASKS,
...
export interface OperationalLearningReport {
  ...
  checkpoints: Array<{ iteration: number; goldenSuccessRate: number; policyVersion: number }>;
}

export class OperationalLearningKernel {
  readonly telemetry: TelemetryStore;
  readonly bridge = new PolicyRuntimeBridge();
  private analyzer: ImprovementAnalyzer;
  private applier: ImprovementApplier;

  constructor(
    private policyStore: LearnedPolicyStore,
    private experience: ExperienceReplayBuffer,
    hotPatch: HotPatchManager,
    dataDir?: string
  ) {
    this.telemetry = new TelemetryStore(dataDir);
    this.analyzer = new ImprovementAnalyzer();
    this.applier = new ImprovementApplier(policyStore, hotPatch);
  }
  ...
  simulateTaskOutcome(
    ...
    const routingBoost =
      (task.category === 'reasoning' && policy.routingBias.planner === 'reasoning' ? 0.12 : 0) +
    ...
    const { operationalSuccess, reward } = evaluateOperationalSuccess({
  ...
  evaluateGoldenSet(policy?: LearnedPolicy): number {
    ...
    for (const task of GOLDEN_TASKS) {
      if (this.simulateTaskOutcome(task, p, { benchmark: true }).operationalSuccess) ok += 1;
    }
    return ok / GOLDEN_TASKS.length;
  }

  async runLoop(iterations: number): Promise<OperationalLearningReport> {
    const checkpoints: OperationalLearningReport['checkpoints'] = [];
    ...
    for (let i = 1; i <= iterations; i++) {
      ...
      if (task.category) {
        ...
        cat.total += 1;
        if (outcome.operationalSuccess) cat.success += 1;
      ...
      if (i % 100 === 0 || i === iterations) {
        ...
        console.log(
          `  [${i}/${iterations}] golden=${(rate * 100).toFixed(1)}% policy=v${this.policyStore.get().version}`
        );
      }
    }
  ...
  private deterministicRoll(taskId: string, version: number, iteration: number): number {
    let h = iteration * 997 + version * 13;
    for (let i = 0; i < taskId.length; i++) h = (h + taskId.charCodeAt(i) * (i + 1)) % 10000;
    return (h % 1000) / 1000;
  }
}
